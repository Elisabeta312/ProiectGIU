...
public class GoatSheepInvisiblePenSpawner : MonoBehaviour
{
    [Header("Animal")]
    ...
    [Tooltip("Colliderul copil care reprezinta tarcul invizibil.")]
    ...
    [Tooltip("Lasa true cand testezi direct din MainGameScene.")]
    public bool allowDirectSceneTesting = true;

    [Tooltip("Ascunde animalul original/template dupa ce spawneaza copiile.")]
    public bool hideTemplateAtRuntime = true;

    [Tooltip("Sterge tarcul invizibil de pe copiile spawnate.")]
    public bool removePenFromSpawnedAnimals = true;

    [Header("Goat/Sheep Roaming")]
    ...
    [Header("Goat/Sheep Animation States")]
    ...
    private void Start()
    {
        ...
        if (penCollider == null)
        {
            Debug.LogWarning(name + " has no goat/sheep invisible pen collider assigned.");
            return;
        }
    ...
    public void SpawnAnimals()
    {
        ...
        if (penCollider == null)
        {
            Debug.LogWarning(name + " cannot spawn goat/sheep because penCollider is missing.");
            return;
        }
        ...
        for (int i = 0; i < spawnCount; i++)
        {
            ...
            RemoveSpawnerScriptsFromSpawnedAnimal(spawnedAnimal);
            RemoveOldRoamersFromSpawnedAnimal(spawnedAnimal);
            ...
            GoatSheepRoamer roamer = spawnedAnimal.AddComponent<GoatSheepRoamer>();
            ...
            roamer.idleStateName = idleStateName;
            roamer.walkStateName = walkStateName;
            roamer.animationFadeTime = animationFadeTime;
        }

        Debug.Log("Spawned " + spawnCount + " goat/sheep animals of type " + spawnId);
    }

    private void RemoveSpawnerScriptsFromSpawnedAnimal(GameObject spawnedAnimal)
    {
        GoatSheepInvisiblePenSpawner goatSheepSpawner = spawnedAnimal.GetComponent<GoatSheepInvisiblePenSpawner>();

        if (goatSheepSpawner != null)
        {
            Destroy(goatSheepSpawner);
        }

        AnimalInvisiblePenSpawner genericInvisiblePenSpawner = spawnedAnimal.GetComponent<AnimalInvisiblePenSpawner>();

        if (genericInvisiblePenSpawner != null)
        {
            Destroy(genericInvisiblePenSpawner);
        }

        TerrestrialAnimalSpawner terrestrialSpawner = spawnedAnimal.GetComponent<TerrestrialAnimalSpawner>();

        if (terrestrialSpawner != null)
        {
            Destroy(terrestrialSpawner);
        }
    }

    private void RemoveOldRoamersFromSpawnedAnimal(GameObject spawnedAnimal)
    {
        GoatSheepRoamer goatSheepRoamer = spawnedAnimal.GetComponent<GoatSheepRoamer>();

        if (goatSheepRoamer != null)
        {
            Destroy(goatSheepRoamer);
        }

        TerrestrialAnimalRoamer terrestrialRoamer = spawnedAnimal.GetComponent<TerrestrialAnimalRoamer>();

        if (terrestrialRoamer != null)
        {
            Destroy(terrestrialRoamer);
        }
    }
    ...
    private void AutoFindPenColliderIfNeeded()
    {
        ...
        foreach (Collider childCollider in childColliders)
        {
            ...
            string lowerName = childCollider.name.ToLower();

            if (lowerName.Contains("pen") ||
                lowerName.Contains("tarc") ||
